...
**Series-type classification:** `classify_series_types.py` consumes `series_index.csv` and labels each series by matching `SeriesDescription` against an ordered regex list ported verbatim from the R `seriesTypes` list (`nonAnat, T13D, AxT1C, T1C, T1, T2star, CubeFLAIR, FLAIR, WandT2, AxT2, OtherT2, DSC, DCE, Trace, eADC, ADC, AvgDC, DWIDTI, FA`), then counts each type per study. **The rule order is load-bearing — first match wins** (that is what keeps `AxT1C` from being swallowed by `T1C`/`T1`, and `CubeFLAIR` by `FLAIR`); `--all-matches` records every rule a series matched for tuning. Matching is **case-insensitive by default** (the R patterns mix case conventions and real descriptions vary); `--case-sensitive` restores literal `grepl(perl=TRUE)` behavior. Outputs `series_types.csv` (per-series, has UIDs) and `series_type_counts.csv` (per-study wide counts, UID/MRN-free and shareable); the console `[gap ]` lines say how many studies lack each type — the direct answer to how many patients have no post-contrast T1.
